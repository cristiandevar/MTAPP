					...
					if (popupObj.isActive()) {
						if (this.docReady(iFrame, times) || (times > MAX_CHECK_LOADED_TIMES_PDF && this.isPDFPopup(iFrame, popupObj))) {
							...
							if (frameDocLoaded || isPdf) {
								...
								var copyOptions = () => {
									var attributeNames = document.documentElement.getAttributeNames().filter( att => att.startsWith("gx-data-"))
									attributeNames.forEach( att => frameDoc.documentElement.setAttribute(att, document.documentElement.getAttribute(att)))
								}
								copyOptions();
