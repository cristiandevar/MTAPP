   ...
   public class employeeinvoicewc : GXWebComponent
   {
      ...
      protected void RF0L2( )
      {
         ...
         if ( ! context.WillRedirect( ) && ( context.nUserReturn != 1 ) )
         {
            ...
            {
               A12EmployeeId = H000L3_A12EmployeeId[0];
               AssignAttri(sPrefix, false, "A12EmployeeId", StringUtil.LTrimStr( (decimal)(A12EmployeeId), 6, 0));
               A39InvoiceModifiedDate = H000L3_A39InvoiceModifiedDate[0];
               n39InvoiceModifiedDate = H000L3_n39InvoiceModifiedDate[0];
               A38InvoiceCreatedDate = H000L3_A38InvoiceCreatedDate[0];
               A21InvoiceDate = H000L3_A21InvoiceDate[0];
               A20InvoiceId = H000L3_A20InvoiceId[0];
               A68InvoiceProductQuantity = H000L3_A68InvoiceProductQuantity[0];
               n68InvoiceProductQuantity = H000L3_n68InvoiceProductQuantity[0];
               A68InvoiceProductQuantity = H000L3_A68InvoiceProductQuantity[0];
               n68InvoiceProductQuantity = H000L3_n68InvoiceProductQuantity[0];
               E130L2 ();
      ...
      protected int subGrid_fnc_Recordcount( )
      {
         ...
         GRID_nRecordCount = H000L5_AGRID_nRecordCount[0];
         pr_default.close(1);
      ...
      protected void define_styles( )
      {
         ...
         {
            context.AddJavascriptSource(StringUtil.RTrim( ((string)Form.Jscriptsrc.Item(idxLst))), "?202311222273483", true, true);
            idxLst = (int)(idxLst+1);
         }
         if ( ! outputEnabled )
         {
      ...
      protected void include_jscripts( )
      {
         if ( nGXWrapped != 1 )
         {
            context.AddJavascriptSource("employeeinvoicewc.js", "?202311222273483", false, true);
         }
      ...
      public override void initialize( )
      {
         ...
         scmdbuf = "";
         H000L3_A12EmployeeId = new int[1] ;
         H000L3_A39InvoiceModifiedDate = new DateTime[] {DateTime.MinValue} ;
         H000L3_n39InvoiceModifiedDate = new bool[] {false} ;
         H000L3_A38InvoiceCreatedDate = new DateTime[] {DateTime.MinValue} ;
         H000L3_A21InvoiceDate = new DateTime[] {DateTime.MinValue} ;
         H000L3_A20InvoiceId = new int[1] ;
         H000L3_A68InvoiceProductQuantity = new short[1] ;
         H000L3_n68InvoiceProductQuantity = new bool[] {false} ;
         H000L5_AGRID_nRecordCount = new long[1] ;
         ...
         pr_default = new DataStoreProvider(context, new GeneXus.Programs.employeeinvoicewc__default(),
            new Object[][] {
                new Object[] {
               H000L3_A12EmployeeId, H000L3_A39InvoiceModifiedDate, H000L3_n39InvoiceModifiedDate, H000L3_A38InvoiceCreatedDate, H000L3_A21InvoiceDate, H000L3_A20InvoiceId, H000L3_A68InvoiceProductQuantity, H000L3_n68InvoiceProductQuantity
               }
               , new Object[] {
               H000L5_AGRID_nRecordCount
               }
            }
         );
         AV17Pgmname = "EmployeeInvoiceWC";
      ...
      private int[] H000L3_A12EmployeeId ;
      private DateTime[] H000L3_A39InvoiceModifiedDate ;
      private bool[] H000L3_n39InvoiceModifiedDate ;
      private DateTime[] H000L3_A38InvoiceCreatedDate ;
      private DateTime[] H000L3_A21InvoiceDate ;
      private int[] H000L3_A20InvoiceId ;
      private short[] H000L3_A68InvoiceProductQuantity ;
      private bool[] H000L3_n68InvoiceProductQuantity ;
      private long[] H000L5_AGRID_nRecordCount ;
   ...
   public class employeeinvoicewc__default : DataStoreHelperBase, IDataStoreHelper
   {
    ...
    private void cursorDefinitions( )
    {
       if ( def == null )
       {
          ...
          def= new CursorDef[] {
              new CursorDef("H000L3", "SELECT T1.[EmployeeId], T1.[InvoiceModifiedDate], T1.[InvoiceCreatedDate], T1.[InvoiceDate], T1.[InvoiceId], COALESCE( T2.[InvoiceProductQuantity], 0) AS InvoiceProductQuantity FROM ([Invoice] T1 LEFT JOIN (SELECT COUNT(*) AS InvoiceProductQuantity, [InvoiceId] FROM [InvoiceDetail] GROUP BY [InvoiceId] ) T2 ON T2.[InvoiceId] = T1.[InvoiceId]) WHERE T1.[EmployeeId] = @AV6EmployeeId ORDER BY T1.[EmployeeId]  OFFSET @GXPagingFrom2 ROWS FETCH NEXT CAST((SELECT CASE WHEN @GXPagingTo2 > 0 THEN @GXPagingTo2 ELSE 1e9 END) AS INTEGER) ROWS ONLY",false, GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK, false, this,prmH000L3,11, GxCacheFrequency.OFF ,true,false )
             ,new CursorDef("H000L5", "SELECT COUNT(*) FROM ([Invoice] T1 LEFT JOIN (SELECT COUNT(*) AS InvoiceProductQuantity, [InvoiceId] FROM [InvoiceDetail] GROUP BY [InvoiceId] ) T2 ON T2.[InvoiceId] = T1.[InvoiceId]) WHERE T1.[EmployeeId] = @AV6EmployeeId ",false, GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK, false, this,prmH000L5,1, GxCacheFrequency.OFF ,true,false )
          };
       }
    }
